...
object DSHenonLozi extends ChaosParameter[DSHenonLozi]
{
	...
	def setup() : Unit = {
		...
		params = params.enqueue(	ParamRange.neighbor(-1.8 , 0.01  , 1) ,
									ParamRange.neighbor( 0.3 , 0.001 , 1) , 0.1 )
					
		params = params.enqueue(	ParamRange.neighbor(-1.8 , 0.01  , 1) ,
									ParamRange.neighbor( 0.5 , 0.001 , 1) , 0.1 )
		
		params = params.enqueue(	ParamRange.neighbor( 1.4 , 0.01  , 1) ,
									ParamRange.neighbor( 0.3 , 0.001 , 1) , 0.1 )
		
		params = params.enqueue(	ParamRange.neighbor( 1.35 , 0.01  , 1) ,
									ParamRange.neighbor( 0.3  , 0.001 , 1) , 0.1 )
			
		params = params.enqueue(	ParamRange.neighbor( 0.2  , 0.01  , 1) ,
									ParamRange.neighbor( 1.01 , 0.001 , 1) , 0.1 )
		for( m<-maps ; p<-params){
